...
case "${DEVICE_NAME}" in
  genymotion)
    ...
    then
      ...
      # Attempt to START the VM if it IS NOT running
      while ! VBoxManage list runningvms | testOutputContains "${DEVICE_ID}" &&
        [ "${ATTEMPT_TO_START}" -lt "${ATTEMPTS_TO_START}" ]
      do
        ((ATTEMPT_TO_START++))
        echo "[${ATTEMPT_TO_START}/${ATTEMPTS_TO_START}] Attempting to start DEVICE_ID: ${DEVICE_ID}"
        /Applications/Genymotion.app/Contents/MacOS/player --vm-name "${DEVICE_ID}" &
        sleep 10
      done
